...
public partial class MainWindow : Window
{
    ...
    private sealed record ProjectExplorerMoveTarget(string FullPath, string RelativePath)
    {
        public override string ToString()
    ...
    private static readonly DataFormat<string> ProjectExplorerDragDataFormat =
        DataFormat.CreateStringApplicationFormat("AvaloniaUIDesigner.ProjectExplorerFile");
    ...
    private static ProjectExplorerNode? FindProjectExplorerFileNode(
        IReadOnlyList<ProjectExplorerNode> roots,
        string fullPath)
    {
        foreach (var node in roots)
        {
            ...
            var descendant = FindProjectExplorerFileNode(node.Children, fullPath);
            if (descendant is not null)
            {
                return descendant;
            }
        ...
        string? workspacePath)
    {
        ...
        try
        {
            return GetProjectExplorerNewFilePath(
                fileName,
    ...
    private static string? GetProjectExplorerMoveFilePath(
        ProjectExplorerNode? node,
        ...
        try
        {
            ...
            var targetIsInWorkspace = string.Equals(
                ...
                || normalizedTargetDirectory.StartsWith(
                    workspacePrefix,
                    StringComparison.OrdinalIgnoreCase);
            var sourceIsInWorkspace = fullCurrentPath.StartsWith(
                workspacePrefix,
                StringComparison.OrdinalIgnoreCase);
            if (!targetIsInWorkspace
                || !sourceIsInWorkspace
                || !File.Exists(fullCurrentPath)
            ...
            return GetProjectExplorerNewFilePath(
                System.IO.Path.GetFileName(fullCurrentPath),
                fullTargetDirectory);
        }
        catch (ArgumentException)
        {
    ...
    private async Task<string?> ShowProjectExplorerAsync()
    {
        ...
        ProjectExplorerNode? pendingProjectExplorerDragNode = null;
        var projectExplorerDragStart = default(Point);
        ...
        void RefreshProjectTree(string? selectedRelativePathOverride = null)
        {
            ...
            string currentPath)
        {
            ...
            void RenameFile()
            {
                var path = GetProjectExplorerRenameFilePath(
                    node,
                    nameEditor.Text,
                    Vm.ProjectWorkspacePath);
                if (path is null)
            ...
            renameButton.Click += (_, _) => RenameFile();
            ...
            renameDialog.Content = new StackPanel
            {
                ...
                Children =
                {
                    new TextBlock { Text = $"Rename {System.IO.Path.GetFileName(currentPath)}." },
                    nameEditor,
                    errorText,
                    new StackPanel
                    {
        ...
        async Task RenameProjectExplorerFileAsync()
        {
            if (list.SelectedItem is not ProjectExplorerNode { IsFile: true } node)
            {
                Vm.StatusText = "Select an AXAML file to rename.";
                return;
            }
            ...
            var newPath = await ShowRenameProjectExplorerFileDialogAsync(node, currentPath);
            ...
            if (!Vm.RefreshProjectWorkspace(out var refreshError))
            {
                Vm.StatusText = $"Renamed the file, but could not refresh Project Explorer: {refreshError}";
                return;
            }
        ...
        async Task<string?> ShowMoveProjectExplorerFileDialogAsync(
            ...
            string currentPath)
        {
            var workspacePath = Vm.ProjectWorkspacePath;
            if (string.IsNullOrWhiteSpace(workspacePath))
            {
                return null;
            }

            var targets = new List<ProjectExplorerMoveTarget>
            {
                new(workspacePath, string.Empty),
            };
            targets.AddRange(Vm.ProjectWorkspaceFolders.Select(folder =>
                new ProjectExplorerMoveTarget(folder.FullPath, folder.RelativePath)));

            var currentDirectory = System.IO.Path.GetDirectoryName(
                System.IO.Path.GetFullPath(currentPath));
            var selectedIndex = targets.FindIndex(target =>
                string.Equals(
                    target.FullPath,
                    currentDirectory,
                    StringComparison.OrdinalIgnoreCase));
            ...
            var moveButton = new Button { Content = "Move", MinWidth = 86 };
            var cancelButton = new Button { Content = "Cancel", MinWidth = 86 };

            void MoveFile()
            {
                if (targetEditor.SelectedItem is not ProjectExplorerMoveTarget target)
                {
                    errorText.Text = "Choose a destination project folder.";
                    return;
                }

                var path = GetProjectExplorerMoveFilePath(
                    node,
                    target.FullPath,
                    Vm.ProjectWorkspacePath);
                if (path is null)
                {
                    errorText.Text = "Choose an existing folder inside the project workspace.";
                    return;
                }

                if (string.Equals(path, currentPath, StringComparison.OrdinalIgnoreCase))
                {
                    errorText.Text = "Choose a different project folder.";
                    return;
                }
                ...
                moveDialog.Close(path);
            }

            moveButton.Click += (_, _) => MoveFile();
            cancelButton.Click += (_, _) => moveDialog.Close(null);
            targetEditor.KeyDown += (_, e) =>
            {
                if (e.Key == Key.Enter)
                {
                    MoveFile();
                ...
                {
                    moveDialog.Close(null);
                    e.Handled = true;
                }
            };
            moveDialog.Opened += (_, _) => targetEditor.Focus();

            moveDialog.Content = new StackPanel
            {
                Margin = new Thickness(16),
                Spacing = 10,
                Children =
                {
                    new TextBlock { Text = $"Move {System.IO.Path.GetFileName(currentPath)} to a project folder." },
                    targetEditor,
                    new TextBlock
                    {
                        Text = "The AXAML file and its recovery backup will move together.",
                        Foreground = Brush.Parse("#64748B"),
                        TextWrapping = TextWrapping.Wrap,
                    },
                    errorText,
            ...
            return await moveDialog.ShowDialog<string?>(dialog);
        }

        async Task<bool> EnsureMoveProjectExplorerSourceReadyAsync(string currentPath)
        {
            var originalTab = Vm.SelectedDocumentTab;
        ...
        async Task MoveProjectExplorerFileAsync(
            ProjectExplorerNode? sourceNode = null,
            string? targetDirectory = null)
        {
            var node = sourceNode ?? list.SelectedItem as ProjectExplorerNode;
            if (node is not { IsFile: true })
            {
                Vm.StatusText = "Select an AXAML file to move.";
                return;
            }
            ...
            {
                newPath = await ShowMoveProjectExplorerFileDialogAsync(node, currentPath);
            }
            else
            {
                newPath = GetProjectExplorerMoveFilePath(
                    node,
                    targetDirectory,
                    Vm.ProjectWorkspacePath);
                if (newPath is null)
                {
                    Vm.StatusText = "The selected Project Explorer destination is unavailable.";
                    return;
                }

                if (string.Equals(newPath, currentPath, StringComparison.OrdinalIgnoreCase))
                {
                    Vm.StatusText = "Choose a different Project Explorer folder.";
                    return;
                }
                ...
                or NotSupportedException)
            {
                ...
                Vm.StatusText = $"Could not move {System.IO.Path.GetFileName(currentPath)}: {exception.Message}";
                return;
            }
            ...
            if (!Vm.RefreshProjectWorkspace(out var refreshError))
            {
                Vm.StatusText = $"Moved the file, but could not refresh Project Explorer: {refreshError}";
                return;
            }
            ...
            {
                RefreshProjectTree();
                Vm.StatusText = $"Moved {System.IO.Path.GetFileName(currentPath)}.";
                return;
            }
        ...
        var moveFileMenu = new MenuItem
        {
            Header = "Move AXAML File...",
            IsEnabled = false,
        };
        ...
        moveFileMenu.Click += async (_, _) => await MoveProjectExplorerFileAsync();
        ...
        projectExplorerContextMenu.Opened += (_, _) =>
        {
            ...
            moveFileMenu.IsEnabled = hasSelectedFile;
        ...
        list.KeyDown += async (_, e) =>
        {
            ...
            {
                await MoveProjectExplorerFileAsync();
                e.Handled = true;
            }
            else if (e.Key == Key.D
                && e.KeyModifiers.HasFlag(KeyModifiers.Control)
        ...
        list.PointerPressed += (_, e) =>
        {
            ...
            if (point.Properties.IsLeftButtonPressed
                && FindProjectExplorerContextMenuNode(e.Source) is { IsFile: true } dragNode)
            {
                pendingProjectExplorerDragNode = dragNode;
                projectExplorerDragStart = e.GetPosition(list);
                e.Pointer.Capture(list);
            }
        };
        list.PointerMoved += async (_, e) =>
        {
            if (pendingProjectExplorerDragNode is not { } dragNode
                || dragNode.FullPath is not { } fullPath)
            {
                return;
            }

            var point = e.GetPosition(list);
            if (Math.Abs(point.X - projectExplorerDragStart.X) < MarqueeThreshold
                && Math.Abs(point.Y - projectExplorerDragStart.Y) < MarqueeThreshold)
            {
                return;
            }

            pendingProjectExplorerDragNode = null;
            e.Pointer.Capture(null);
            var data = new DataTransfer();
            data.Add(DataTransferItem.Create(ProjectExplorerDragDataFormat, fullPath));
    ...
    private async void OnDeclaredCustomPropertyFourValueClicked(
        object? sender,
        Avalonia.Interactivity.RoutedEventArgs e)
    {
        if (_isApplyingDeclaredCustomPropertyEdit
            || sender is not Button { Tag: string propertyName }
            || GetDeclaredCustomPropertyState(propertyName) is not
                { UsesFourValueEditor: true } state)
        {
            return;
        }

        await ShowDeclaredCustomPropertyFourValueDialogAsync(state);
        QueueDeclaredCustomPropertySummaryRefresh();
    }

    private async Task ShowDeclaredCustomPropertyFourValueDialogAsync(
        DesignerCustomPropertyValueState state)
    {
        if (Vm is null)
        {
        ...
        var valueEditor = new DesignerCustomPropertyFourValueEditor(state);
        var errorText = new TextBlock
        {
            Foreground = Brushes.IndianRed,
            TextWrapping = TextWrapping.Wrap,
        ...
        var directionHint = state.Type == DesignerCustomPropertyType.Thickness
        ...
        var content = new Grid
        {
            ...
            Children =
            {
                new TextBlock
                {
                    Text = $"Choose a local {state.TypeLabel} value for {state.DisplayName}.",
                    FontWeight = FontWeight.SemiBold,
                },
                new TextBlock
                {
                    Text = $"Order: {directionHint}. Current effective value: {state.DisplayValue}",
        ...
        CustomPropertyBindingEditorState state)
    {
        ...
        var content = new Grid
        {
            ...
            Children =
            {
                ...
                new TextBlock
                {
                    Text = "Use a dotted CLR path such as Order.Total. Fallback cannot contain braces, quotes, or pipes.",
                    Foreground = Avalonia.Media.Brushes.SlateGray,
                    TextWrapping = Avalonia.Media.TextWrapping.Wrap,
                },
                errorText,
