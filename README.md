...
- **Arrange 키보드 단축키**: 캔버스에서 다중 선택 후 `Ctrl+Shift+Left/Right/Up/Down`으로 선택 컨트롤을 좌·우·상·하 경계에 정렬하고 기존 Arrange Undo/AXAML 흐름을 그대로 사용
...
75p. 캔버스에서 두 개 이상의 컨트롤을 선택한 뒤 `Ctrl+Shift+Left/Right/Up/Down`을 누르면 각각 선택 영역의 좌·우·상·하 경계에 맞춰 정렬합니다. Object Tree에 포커스가 있으면 방향키 탐색을 우선하며, 정렬 결과는 하나의 Undo 작업과 AXAML에 반영됩니다.
