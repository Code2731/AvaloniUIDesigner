...
- **Typed Custom Properties**: Component Pack의 `propertyDefinitions`로 String·Boolean·Integer·Double·Color·Enum·Thickness·CornerRadius·GridLength, 숫자 범위·Enum 옵션, 표시 이름·카테고리·설명을 선언하고 Inspector 카테고리 그룹·검색·선택·스핀·색상·4방향·트랙 크기 편집기와 공통 검증을 적용
...
- **Project Workspace Explorer**: `File > Open Project Folder...` 또는 `File > Open Project or Solution... (Ctrl+Shift+J)`로 작업 폴더·`.csproj`·`.sln`·`.slnx` 파일을 선택하면 프로젝트 디렉터리를 workspace root로 열고, `Project Explorer... (Ctrl+Shift+P)`에서 폴더·`.axaml`·`.xaml` 파일 계층을 탐색하고 Enter 또는 파일 더블클릭으로 새 문서 탭에 엽니다. 폴더는 `[+]`/`[-]`와 Enter·더블클릭으로 접고 펼치며 Left/Right 방향키로 접기·펼치기·부모/첫 자식 이동을 수행하고 검색 시 일치 파일의 조상 폴더를 유지합니다. 명시적인 tree item template가 깊이·폴더 상태·상대 경로를 표시하며, `Ctrl+N` 또는 context menu에서 `UserControl`/`Window` 템플릿을 선택해 안전하게 새 AXAML 파일을 만들고 즉시 편집 탭으로 엽니다. `Ctrl+Shift+N` 또는 context menu에서 선택 폴더·파일의 부모·workspace root에 새 폴더를 만들 수 있고, 생성 후 트리에서 바로 확인할 수 있습니다. AXAML 파일을 선택하면 `Ctrl+Shift+D` 또는 context menu에서 원본과 backup을 함께 복제하고 새 문서 탭으로 열 수 있으며 dirty 원본은 저장·폐기·취소를 먼저 확인합니다. `Ctrl+Shift+M` 또는 context menu로 다른 폴더를 선택해 이동하거나 파일 행을 폴더 행으로 직접 drag-and-drop할 수 있으며, 두 경로 모두 같은 dirty 보호·충돌 검증·문서 경로 연동을 사용합니다. 외부 파일 관리자에서 `.axaml`/`.xaml` 파일을 root 또는 폴더로 drag-and-drop하면 원본을 유지한 채 프로젝트로 복사합니다. 폴더를 선택하면 `F2` 또는 context menu의 `Rename Folder...`로 이름을 바꿀 수 있으며, 하위 문서 탭·닫힌 탭 기록·Recent Files·외부 변경 경로·접기 상태도 새 경로로 유지됩니다. `Delete`/`Backspace` 또는 context menu의 `Delete Empty Folder...`는 내용이 없는 폴더만 확인 후 삭제해 파일과 하위 폴더의 실수 삭제를 차단합니다. AXAML 파일을 선택하면 `F2` 또는 context menu로 이름을 바꿀 수 있고, `Delete`/`Backspace` 또는 context menu로 dirty 확인 후 파일과 backup을 삭제할 수 있습니다. 열려 있는 탭·닫힌 탭 기록·Recent Files·dirty 저장 경로도 파일 작업에 맞춰 일관되게 유지됩니다. 우클릭한 행을 먼저 선택한 뒤 선택한 파일·폴더에 context menu의 relative/full path 복사, `Ctrl+C` full path 복사, `Open in File Manager` 위치 열기를 적용합니다. Explorer dialog는 크기 조절이 가능하고 마지막 너비·높이를 session에 저장해 다음 실행에서 복원합니다. 현재 문서 파일은 다시 열 때 자동으로 reveal하고 폴더 접기 상태도 저장·복원합니다. `bin`·`obj`·`.git` 등 생성 폴더는 제외하며 선택한 프로젝트 폴더와 파일 목록은 다음 실행에도 복원됩니다. 최근 workspace는 최대 8개까지 저장되어 `Open Recent Projects... (Ctrl+Shift+L)`에서 이름·경로로 검색하고 빠르게 전환할 수 있으며, 누락된 폴더는 선택 시 목록에서 제거됩니다. `Close Project Workspace (Ctrl+Shift+X)`는 Explorer 연결만 닫고 열린 AXAML 탭과 최근 workspace 기록은 보존합니다.
...
- **Paired AXAML Code-behind Creation**: Project Explorer의 새 AXAML 파일 대화상자에서 선택적인 `x:Class`와 `.axaml.cs` paired file을 함께 생성하고, Window/UserControl base type·namespace·`InitializeComponent()`를 일치시킵니다. paired file 충돌이나 쓰기 실패 시 부분 생성물을 정리합니다.
- **Paired AXAML Code-behind Management**: Project Explorer의 AXAML 행에 `paired` 상태를 표시하고, rename·move·delete 시 같은 이름의 `.axaml.cs`와 함께 처리합니다. duplicate에서는 새 파일명에 맞춰 AXAML `x:Class`와 code-behind partial class·constructor를 함께 재작성해 동일한 타입 충돌을 피합니다.
...
- **Project Workspace Build**: `File > Build Project Workspace...`에서 workspace의 `.csproj`·`.sln`·`.slnx`를 선택해 `dotnet build`를 실행하고 실시간 stdout/stderr, 성공·실패·취소 결과를 전용 로그 창에서 확인합니다. `bin`·`obj`·`.git`·`.vs` 아래의 생성물은 build target에서 제외합니다.
...
- **Design Layout Diagnostics**: 저장 전 현재 문서의 아트보드 크기·요소 bounds·중복 이름·부모 참조·컨테이너 layout·순환 계층을 검사하고, 오류/경고를 필터링해 확인하며 대상 컨트롤을 Canvas에서 바로 선택합니다.
...
12ae. Project Explorer에서 `New AXAML File...`을 실행하면 기본적으로 `x:Class`가 들어간 AXAML과 같은 이름의 `.axaml.cs`를 함께 생성합니다. `UserControl`/`Window` 선택, `Views.NewView` 타입명, paired file 충돌 검사를 적용하며 checkbox를 해제하면 순수 AXAML만 만들 수 있습니다.
12af. Project Explorer의 AXAML 행에 `paired` 표시가 있으면 같은 이름의 `.axaml.cs`가 연결된 상태입니다. `F2` rename, `Move`, `Duplicate`, `Delete`는 paired file을 함께 보호·이동·복제·삭제하며, duplicate는 새 파일명 기준으로 `x:Class`와 partial class/constructor 이름을 갱신합니다.
...
12b4. `File > Build Project Workspace...`를 실행하면 workspace에서 찾은 `.csproj`·`.sln`·`.slnx` 중 하나를 고르고 `dotnet build --nologo`를 실행합니다. 여러 대상은 이름·경로 검색으로 고를 수 있으며, 빌드 중 `Cancel Build`는 dotnet 프로세스 트리를 종료하고 로그 창은 stdout/stderr와 종료 상태를 보존합니다.
12b5. build 로그에서 `file(line,column): error|warning code: message` 형식의 진단을 발견하면 별도 목록에 표시합니다. 진단을 더블클릭하거나 선택 후 Enter하면 AXAML/XAML은 디자이너 새 탭으로 열고, 그 외 파일은 OS 기본 연결 프로그램으로 열며 줄·열 위치를 status bar에 표시합니다.
...
12c. `File > Validate Layout...` 또는 Design Toolbar의 `Layout Check`를 실행하면 저장되지 않은 현재 문서를 기준으로 아트보드 크기, 요소 bounds, 이름, 부모 관계와 계층을 검사합니다. `All`·`Errors`·`Warnings` 필터, 선택 진단 복사, 더블클릭/Enter 또는 `Select in Canvas`로 대상 요소 이동을 지원합니다.
...
75cs. Project Explorer에서 AXAML 파일을 선택하고 `Delete`/`Backspace` 또는 context menu의 `Delete AXAML File...`을 실행하면 dirty 탭의 기존 Save/Discard/Cancel 보호를 거친 뒤 파일과 `.bak` 복구 파일을 삭제합니다. 삭제 확인 후 관련 열린 탭과 닫힌 탭 기록, Recent Files, 외부 변경 알림 상태를 정리하고 Project Explorer tree를 갱신하며, 대상이 유일한 탭이면 빈 문서 탭을 남겨 디자이너가 항상 편집 가능한 상태를 유지합니다.
75ct. Project Explorer에서 `Ctrl+Shift+N` 또는 context menu의 `New Folder...`을 실행하면 선택한 폴더·AXAML 파일의 부모·workspace root에 새 폴더를 생성합니다. 폴더 구분자·잘못된 문자·`.`/`..`·중복 파일/폴더명은 거부하고, 생성 후 Project Explorer tree와 접기 상태를 유지하면서 새 폴더를 선택합니다.
...
75cy. Project Explorer AXAML 파일 행을 기존 폴더 행으로 drag-and-drop하면 `Move AXAML File...`과 같은 대상 검증·dirty 보호·source/`.bak` 이동·열린/닫힌 탭 경로 갱신을 수행합니다. 유효하지 않은 대상, 같은 폴더, 기존 파일·backup 충돌은 drop cursor를 거부하고 원본을 유지합니다.
75cz. Project Explorer AXAML 파일을 workspace root 경로 표시 또는 목록 빈 공간으로 drag-and-drop하면 프로젝트 루트로 이동합니다. 폴더 대상과 동일하게 dirty 보호·source/`.bak` 이동·탭 경로 갱신과 대상 충돌 검증을 적용합니다.
75da. 외부 파일 관리자에서 `.axaml`/`.xaml` 파일을 Project Explorer의 workspace root 또는 폴더로 drag-and-drop하면 원본은 유지한 채 프로젝트로 복사합니다. 여러 파일을 함께 가져올 수 있으며, 지원하지 않는 파일·폴더·workspace 밖 대상·기존 파일 또는 backup 충돌은 drop cursor에서 거부하고 복사 중 실패하면 앞서 복사한 파일도 롤백합니다.
75db. `File > Open Project or Solution...` 또는 `Ctrl+Shift+J`에서 `.csproj`·`.sln`·`.slnx` 파일을 선택하면 해당 파일의 디렉터리를 Project Workspace root로 열어 기존 Explorer·watcher·session 복원 흐름을 그대로 사용합니다. 지원하지 않는 확장자·누락 파일·로컬 경로로 변환할 수 없는 항목은 workspace를 변경하지 않고 상태바에 안내합니다.
75dc. 최근에 연 Project Workspace root는 최대 8개까지 session workspace 저장소에 기록됩니다. `Open Recent Projects... (Ctrl+Shift+L)` 또는 Design Toolbar의 `Recent Projects`에서 이름·경로를 검색하고 Enter로 전환하며, 존재하지 않는 경로는 `Missing`으로 표시한 뒤 선택 시 최근 목록에서 제거합니다.
75dd. `Close Project Workspace (Ctrl+Shift+X)` 또는 Design Toolbar의 `Close Workspace`는 현재 Project Explorer·watcher 연결과 workspace 파일 목록만 닫고, 열린 문서 탭·최근 workspace 기록은 유지합니다. workspace가 닫히면 Explorer·Refresh·Close 명령은 비활성화되고 다른 폴더나 프로젝트를 다시 열 수 있습니다.
...
75dl. Project Explorer 새 AXAML 파일 생성은 선택한 root template와 dotted `x:Class`를 바탕으로 `.axaml`/`.xaml`과 `.axaml.cs` paired file을 원자적으로 준비합니다. code-behind를 생성할 때 namespace·partial class·Window/UserControl base type·`InitializeComponent()`가 서로 일치하며, 두 번째 파일 생성이 실패하면 이미 생성한 첫 번째 파일을 rollback합니다.
75dm. Project Explorer는 AXAML 옆에 존재하는 `.axaml.cs`를 `paired` 상태로 표시하고, AXAML rename·move·delete 작업에서 backup과 함께 code-behind를 동반 처리합니다. paired AXAML duplicate는 새 파일명으로 `x:Class`, partial class, constructor를 함께 바꾸며 대상 AXAML·backup·code-behind 충돌을 사전에 차단하고 부분 작업을 rollback합니다.
...
75dr. `Build Project Workspace...`는 현재 workspace 하위의 실제 `.csproj`·`.sln`·`.slnx`만 정렬해 선택 목록으로 제공하고, 생성 폴더를 제외한 뒤 `dotnet build --nologo`를 workspace 기준으로 실행합니다. 전용 로그 창은 표준 출력과 오류를 실시간으로 보여주고, 빌드 중 닫기를 차단하며, 취소 시 전체 프로세스 트리를 종료하고 최종 상태를 status bar에 남깁니다.
75ds. build 출력은 표준 MSBuild `path(line,column): severity code: message` 패턴을 진단 모델로 추출해 severity·code·message·build working directory 기준 상대 경로를 목록에 표시합니다. 진단 항목을 더블클릭하면 AXAML/XAML은 디자이너 문서 탭으로 열고, 다른 소스 파일은 OS 기본 편집기로 열며, 파일 누락·접근 실패는 기존 상태를 보존한 채 status bar에 알립니다.
...
75dx. `DesignerLayoutValidator`는 현재 문서 snapshot을 변경하지 않고 artboard size, finite/positive bounds, root의 artboard 이탈, duplicate/empty name, missing parent, 알려진 container의 parent layout mismatch, parent cycle을 error/warning으로 분류합니다. `Layout Diagnostics` 대화상자는 전체 severity 개수를 유지한 채 filter를 적용하고, 선택 항목을 clipboard로 복사하거나 Canvas의 해당 control을 선택합니다.
...
- v2.67: Project Explorer AXAML 파일 삭제와 dirty 탭 보호·경로 정리 추가
- v2.68: Project Explorer 새 폴더 생성과 선택·경로 검증 추가
...
- v2.73: Project Explorer AXAML 파일 drag-and-drop 이동 추가
- v2.74: Project Explorer workspace root drag-and-drop 대상 추가
- v2.75: Project Explorer 외부 AXAML 파일 import drag-and-drop 추가
- v2.76: Avalonia 프로젝트/솔루션 파일로 Project Workspace 열기 추가
- v2.77: 최근 Project Workspace 검색·빠른 전환 추가
- v2.78: Project Workspace 닫기와 열린 문서 보존 추가
- v2.79: 명령줄 시작 경로에서 AXAML·Project Workspace 자동 열기 추가
...
- v2.87: Project Explorer 새 AXAML paired code-behind 생성과 rollback 보호 추가
- v2.88: Project Explorer paired AXAML code-behind 상태 표시와 파일 작업 동기화 추가
...
- v2.93: Project Workspace `dotnet build` 선택·실시간 로그·취소 흐름 추가
- v2.94: Project build 진단 목록과 파일 열기 연결 추가
- v2.95: Project build dirty 문서 저장 preflight 추가
...
- v3.05: Preview 상단의 `Reset Preview`로 입력·토글·선택과 로그를 초기화하고 최신 디자인 값 및 샘플 데이터로 테스트를 다시 시작합니다. Preview 스크롤은 처음으로 돌아가며 창 크기와 테마는 유지됩니다.
- v3.06: Preview Canvas 준비가 실패하면 기존 컨트롤·입력 상태·제목·배경과 마지막 정상 Reset 스냅샷을 유지하도록 갱신 순서를 개선했습니다.
- v3.07: 자동 Preview 갱신과 수동 열기의 오류를 상태바에 표시해 Preview 실패가 문서 편집 이벤트로 전파되지 않도록 보완했습니다.
- v3.08: Preview의 `Live updates`를 끄면 테스트 입력을 유지하며 자동 갱신을 일시정지합니다. 다시 켜거나 Reset을 누르면 최신 대기 디자인을 적용합니다.
- v3.09: 잘못된 대기 디자인으로 Reset이 실패하면 Preview에 원인을 표시합니다. 수정된 디자인을 적용하는 재시도와 자동 갱신 재개 실패를 Headless 테스트로 검증했습니다.
- v3.10: Preview 오류 안내를 별도 줄바꿈·스크롤 영역에 배치하고 정상 갱신 후 이전 오류를 지우도록 개선했습니다.
- v3.11: Reset·갱신으로 교체된 이전 컨트롤의 이벤트가 현재 Preview 로그에 섞이지 않도록 차단했습니다.
- v3.12: Draft AXAML에도 루트·중첩 컨트롤의 잠금 메타데이터를 기록해 저장·불러오기와 반복 왕복 후에도 잠금 상태를 유지합니다.
- v3.13: Design-only 커스텀 컨트롤의 숨김·비활성·히트 테스트·흐름 방향·커서 상태를 Draft AXAML에 보존하면서 플레이스홀더 전용 외형은 출력하지 않도록 개선했습니다.
...
- v3.33: Component Pack 사용자 정의 속성에 `Thickness`·`CornerRadius` 타입과 Avalonia 1/2/4값 정규화, Inspector 전용 대화상자·일괄 4방향 스핀 편집기, 쉼표 포함 binding fallback AXAML 왕복을 추가합니다.
- v3.34: Component Pack 사용자 정의 속성에 `GridLength` 타입과 `Auto`·Pixel·Star 정규화, Inspector 전용 단위 편집 대화상자와 일괄 단위·숫자 편집기, 스타일·binding fallback·Draft/Preview/Undo 보존을 추가합니다.
