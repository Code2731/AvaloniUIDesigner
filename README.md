...
75cq. Project Explorer의 `New AXAML File...` 또는 목록 포커스 상태의 `Ctrl+N`은 선택 폴더에 `.axaml` 파일을 생성합니다. 파일을 우클릭하면 같은 폴더, 폴더를 우클릭하면 해당 폴더, 선택이 없으면 workspace root를 사용하며, 파일명은 폴더 구분자·잘못된 확장자·중복을 거부하고 생성 직후 새 document tab으로 엽니다.
