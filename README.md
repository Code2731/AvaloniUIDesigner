...
- **Command-line Startup Paths**: 파일 탐색기나 터미널에서 `.axaml`·`.xaml` 문서, 폴더, `.csproj`·`.sln`·`.slnx` 경로를 인수로 전달해 실행하면 세션 복원 후 해당 문서 또는 Project Workspace를 바로 엽니다. 지원하지 않는 경로와 읽기 실패는 기존 세션을 유지한 채 상태바에 안내합니다.
